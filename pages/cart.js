...
const Cart = () =>{
    ...
    useEffect(()=> {
        const cartLocal = JSON.parse(localStorage.getItem('__cart01__freshveg'))
        if(cartLocal && cartLocal.length > 0){
            let newArr = []
            const updateCart = async () => {
                for(const item of cartLocal){
                    console.log(item)
                    const res = await getData(`product/item/${item.slug}`)
                    console.log(res)
                    const { _id, title, images, price, inStock, sold , slug } = res.product
                    if(inStock > 0){
                        newArr.push({
                            _id, title, images, price, inStock, sold, slug,
